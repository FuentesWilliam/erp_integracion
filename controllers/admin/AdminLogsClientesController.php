...
class AdminLogsClientesController extends ModuleAdminController
{
    ...
    public function initContent()
    {
        ...
        foreach ($logs as $log) {
            if (strpos($log['message'], 'cliente') !== false) {
                $filteredLogs['data'][] = $log;
            }
        }

        // Asignar datos a Smarty
        //$actionUrl = $this->context->link->getAdminLink('AdminLogsClientes') . '&log_date=' . urlencode($date);
        $actionUrl = $this->context->link->getAdminLink('AdminLogsClientes', true, [], ['log_date' => $date]);
    ...
    public function postProcess()
    {
        ...
        if ($date && $this->context->cookie->__get('log_date') !== $date) {
            ...
            //Tools::redirectAdmin($this->context->link->getAdminLink('AdminLogsClientes', true, [], ['log_date' => $date]));
            Tools::redirectAdmin(
                $this->context->link->getAdminLink('AdminLogsClientes', true, [], ['log_date' => $date])
            );
        }
    }
